...
export function useContributions(user?: UserProfile | null, target?: { type: ContributionTargetType; id: string }) {
  ...
  return { contributions, audit, loading, error, stats, refresh, createContribution, approveContribution: (id: string) => review(id, 'approved'), rejectContribution: (id: string, reason: string) => review(id, 'rejected', reason) };
}
