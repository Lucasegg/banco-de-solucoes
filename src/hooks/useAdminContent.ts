...
import { AdminContentRepository, type AdminContentKind, type AdminContentRecord } from '../repositories/adminContent';

export function useAdminContent(kind: AdminContentKind, enabled: boolean, page: number) {
  ...
  useEffect(() => {
    ...
    void repository.list(kind, page).then((result) => { if (!active) return; setState((value) => result.ok ? { ...value, records: result.data.records, total: result.data.total, loading: false } : { ...value, loading: false, error: result.message }); });
    return () => { active = false; };
  }, [enabled, kind, page, repository, state.refresh]);
