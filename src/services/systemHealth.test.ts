...
test('baseline correto aceita Sprint 26 pendente',()=>{const state=parseMigrationList('20260717250000 | 20260717250000 |\n20260717251000 | 20260717251000 |\n20260717260000 | |');assert.deepEqual(validateMigrationBaseline(state),[]);assert.match(baselineSummary(state,[]),/Sprint 26: pendente/);});
...
test('workflow executa baseline antes de db push e nunca faz repair',()=>{const workflow=readFileSync(new URL('../../.github/workflows/deploy.yml',import.meta.url),'utf8');const baseline=workflow.indexOf('Check migration baseline');const migrate=workflow.indexOf('Apply database migrations');const health=workflow.indexOf('Validate production database health');assert.ok(baseline>0&&migrate>baseline&&health>migrate);assert.doesNotMatch(workflow,/migration repair/);assert.match(workflow,/migrate-and-health:\n    if: github\.event_name == 'push'/);assert.match(workflow,/deploy:[\s\S]*needs: migrate-and-health/);assert.doesNotMatch(workflow,/VITE_SUPABASE_(ACCESS_TOKEN|SERVICE_ROLE_KEY|DB_PASSWORD)/);});
