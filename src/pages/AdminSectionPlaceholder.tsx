...
export function AdminSectionPlaceholder({ title, onBack }: { title: string; onBack: () => void }) {
  ...
  return <section className="mx-auto max-w-2xl rounded-3xl border border-line bg-white p-8 text-center shadow-sm dark:border-slate-700 dark:bg-slate-900">
    ...
    <p className="mt-3 text-muted dark:text-slate-300">{t('admin.placeholder')}</p>
    <button onClick={onBack} className="mt-6 rounded-full bg-slate-950 px-5 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 focus:outline-none focus:ring-2 focus:ring-teal-400 dark:bg-teal-400 dark:text-slate-950">{t('admin.backDashboard')}</button>
  </section>;
}
