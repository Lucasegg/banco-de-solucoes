...
export function ProblemDetails({ id, onNavigate }: { id: string; onNavigate: (page: string) => void }) {
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[1fr_360px]">
      <article className="overflow-hidden rounded-[2rem] border border-line bg-white shadow-sm">
        ...
        <div className="p-8">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <ReportContentDialog targetType="problem" targetId={problem.id} onLoginRequired={() => onNavigate('login')} />
      ...
      <div className="lg:col-span-2 space-y-6"><ProblemTimeline problemId={problem.id} canPublish={permissions.canPublishOfficialUpdate} onStatusChange={(status) => setProblem((current) => current ? { ...current, status } : current)} /><ContributionHistory targetType="problem" targetId={problem.id} /><ItemReactionBar target={{ kind: 'problem', id: problem.id }} /><DiscussionList title={t('details.problemDiscussion')} comments={discussion.comments} currentUserId={discussion.currentUserId} reputations={discussion.reputations} reactionSummaries={discussion.reactionSummaries} pendingReactions={discussion.pendingReactions} storageError={discussion.storageError} onToggleReaction={discussion.toggleReaction} onComment={(content) => discussion.addComment(content)} onEdit={discussion.editComment} onDelete={discussion.deleteComment} /></div>
    </section>
  );
}
...
export function SolutionDetails({ id, onNavigate }: { id: string; onNavigate: (page: string) => void }) {
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[1fr_360px]">
      ...
      <div className="lg:col-span-2 space-y-6"><ContributionHistory targetType="solution" targetId={solution.id} /><ItemReactionBar target={{ kind: 'solution', id: solution.id }} /><DiscussionList title={t('details.solutionDiscussion')} comments={discussion.comments} currentUserId={discussion.currentUserId} reputations={discussion.reputations} reactionSummaries={discussion.reactionSummaries} pendingReactions={discussion.pendingReactions} storageError={discussion.storageError} onToggleReaction={discussion.toggleReaction} onComment={(content) => discussion.addComment(content)} onEdit={discussion.editComment} onDelete={discussion.deleteComment} /></div>
    </section>
  );
}
