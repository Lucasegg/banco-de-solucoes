...
export function Home({ onNavigate }: HomeProps) {
  ...
  return <div className="space-y-14">
    ...
    <section aria-labelledby="first-contribution" className="grid gap-8 rounded-[2rem] border border-sky-100 bg-sky-50 p-6 sm:p-8 md:grid-cols-[1fr_auto] md:items-center">
      <div className="min-w-0"><p className="text-sm font-semibold text-sky-800">{t('home.guide.eyebrow')}</p><h2 id="first-contribution" className="mt-2 text-3xl font-semibold">{t('home.guide.title')}</h2><p className="mt-3 leading-7 text-slate-700">{t('home.guide.description')}</p><ul className="mt-4 list-disc space-y-2 pl-5 text-sm leading-6 text-slate-700"><li>{t('home.guide.problem')}</li><li>{t('home.guide.solution')}</li><li>{t('home.guide.sources')}</li><li>{t('home.guide.moderation')}</li></ul></div>
      <div className="flex flex-col gap-3"><Action page={isAuthenticated ? 'novo-problema' : 'register'} onNavigate={onNavigate} primary>{isAuthenticated ? t('home.startContribution') : t('home.createAccount')}</Action><Action page="contact" onNavigate={onNavigate}>{t('home.requestSupport')}</Action></div>
    </section>
