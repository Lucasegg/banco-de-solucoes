...
export class NotificationRealtimeSubscription {
  ...
  private startPolling(){if(this.stopped||this.healthy||this.timer||this.deps.document.visibilityState!=='visible')return;void this.reconcile();this.timer=this.deps.setInterval(()=>void this.reconcile(),this.pollMs);}
