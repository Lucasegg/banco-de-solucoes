...
import type { Comment as DiscussionComment, CommentReport, CommentReaction } from '../../types/discussion';
...
const selectColumns = 'id,user_id,parent_id,problem_id,solution_id,content,edited,deleted,visibility,best_answer,created_at,updated_at,profiles(display_name,username,avatar_url)';
...
export function parseCommentRow(value: unknown): CommentRow | null {
  ...
  if (!isString(value.id) || !isString(value.user_id) || !isString(value.content) || typeof value.edited !== 'boolean' || typeof value.deleted !== 'boolean' || typeof value.best_answer !== 'boolean' || !isString(value.created_at) || !isString(value.updated_at)) return null;
...
export function normalizeComment(value: unknown): DiscussionComment | null {
  ...
  return { id: value.id, parentId: value.parentId, targetType: value.targetType, targetId: value.targetId, authorId: value.authorId, authorName: value.authorName, authorAvatarUrl: isString(value.authorAvatarUrl) ? value.authorAvatarUrl : null, content: value.content, createdAt: value.createdAt, updatedAt: value.updatedAt, edited: value.edited, deleted: value.deleted, visibility: isContentVisibility(value.visibility) ? value.visibility : value.deleted ? 'removed' : 'visible', bestAnswer: value.bestAnswer, reports: value.reports };
}
...
export function isReaction(value: unknown): value is CommentReaction { if (!isRecord(value)) return false; return isString(value.id) && isString(value.commentId) && isString(value.userId) && (value.type === 'like' || value.type === 'support' || value.type === 'interesting' || value.type === 'needsEvidence') && isString(value.createdAt); }
export function isReactionArray(value: unknown): value is CommentReaction[] { return Array.isArray(value) && value.every(isReaction); }
