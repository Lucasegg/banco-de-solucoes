...
export type CommentInput = { authorId: string; content: string } & ({ problemId: string; solutionId?: never } | { solutionId: string; problemId?: never });
...
const selectColumns = 'id,author_id,problem_id,solution_id,content,created_at,updated_at,profiles(name,username,avatar_url)';
...
function parseProfile(value: unknown): ProfileRow | null {
  ...
  return { name: isString(item.name) ? item.name : null, username: isString(item.username) ? item.username : null, avatar_url: isString(item.avatar_url) ? item.avatar_url : null };
}
export function parseCommentRow(value: unknown): CommentRow | null {
  if (!isRecord(value)) return null;
  if (!isString(value.id) || !isString(value.author_id) || !isString(value.content) || !isString(value.created_at) || !isString(value.updated_at)) return null;
  ...
  return { id: value.id, author_id: value.author_id, problem_id: problemId, solution_id: solutionId, content: value.content, created_at: value.created_at, updated_at: value.updated_at, profiles: parseProfile(value.profiles) };
}
export function mapCommentRowToDomain(row: CommentRow): Comment {
  const profile = parseProfile(row.profiles);
  return { id: row.id, authorId: row.author_id, authorName: profile?.name || profile?.username || 'Usuário', authorAvatarUrl: profile?.avatar_url ?? null, problemId: row.problem_id, solutionId: row.solution_id, content: row.content, createdAt: row.created_at, updatedAt: row.updated_at };
}
...
export class SupabaseCommentRepository {
  ...
  async create(input: CommentInput): Promise<RepositoryResult<Comment>> { const content = validateContent(input.content); if (!content.ok) return content; const payload = 'problemId' in input ? { author_id: input.authorId, problem_id: input.problemId, content: content.data } : { author_id: input.authorId, solution_id: input.solutionId, content: content.data }; const { data, error } = await this.client.from('comments').insert(payload).select(selectColumns).single(); if (error) return { ok: false, message: errorMessage(error, 'Não foi possível criar o comentário.') }; const row = parseCommentRow(data); return row ? { ok: true, data: mapCommentRowToDomain(row) } : { ok: false, message: 'Supabase retornou comentário criado em formato inválido.' }; }
