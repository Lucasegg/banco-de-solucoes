...
const selectColumns = 'id,author_id,parent_id,problem_id,solution_id,content,edited,deleted,visibility,best_answer,reports,created_at,updated_at,profiles(display_name,username,avatar_url)';
...
export function parseCommentRow(value: unknown): CommentRow | null {
  ...
  if (!isString(value.id) || !isString(value.author_id) || !isString(value.content) || typeof value.edited !== 'boolean' || typeof value.deleted !== 'boolean' || typeof value.best_answer !== 'boolean' || !Array.isArray(value.reports) || !value.reports.every(isCommentReport) || !isString(value.created_at) || !isString(value.updated_at)) return null;
...
export class SupabaseCommentRepository {
  ...
  async listReported(): Promise<RepositoryResult<Comment[]>> { const { data, error } = await this.client.from('comments').select(selectColumns).neq('reports', '[]').order('updated_at', { ascending: false }); if (error) return { ok: false, message: errorMessage(error, 'Não foi possível listar comentários reportados.') }; return mapRows(data as unknown[] | null); }
  async create(input: CommentInput): Promise<RepositoryResult<Comment>> { const content = validateContent(input.content); if (!content.ok) return content; const payload = 'problemId' in input ? { author_id: input.authorId, parent_id: input.parentId ?? null, problem_id: input.problemId, content: content.data } : { author_id: input.authorId, parent_id: input.parentId ?? null, solution_id: input.solutionId, content: content.data }; const { data, error } = await this.client.from('comments').insert(payload).select(selectColumns).single(); if (error) return { ok: false, message: errorMessage(error, 'Não foi possível criar o comentário.') }; const row = parseCommentRow(data); return row ? { ok: true, data: mapCommentRowToDomain(row) } : { ok: false, message: 'Supabase retornou comentário criado em formato inválido.' }; }
