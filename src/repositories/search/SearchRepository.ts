...
export type SolutionSearchFilters = BaseFilters & { organization: string; problemId: string; evidence: boolean; impact: boolean };
...
export class SupabaseSearchRepository {
  ...
  async searchSolutions(filters: SolutionSearchFilters): Promise<{ ok: true; data: SearchResponse } | { ok: false; message: string }> {
    const { data, error } = await this.client.rpc('search_solutions', { p_query: nullable(filters.q), p_category: nullable(filters.category), p_organization: nullable(filters.organization), p_tags: filters.tags.length ? filters.tags : null, p_created_from: date(filters.from), p_created_to: date(filters.to, true), p_problem_id: nullable(filters.problemId), p_favorites_only: filters.favorites, p_authored_only: filters.mine, p_has_evidence: filters.evidence || null, p_has_impact_metric: filters.impact || null, p_sort: filters.sort, p_limit: 20, p_offset: (filters.page - 1) * 20 });
    if (error) return { ok: false, message: safeDatabaseMessage(error, 'Não foi possível concluir a busca.') };
    const items = asRows(data); return { ok: true, data: { items, total: items[0]?.total_count ?? 0 } };
  }
}
