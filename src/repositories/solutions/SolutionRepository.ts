...
const safeCategory = (value: string): SolutionCategory => { const normalized=value.replace(/\s+/g,' ').trim(); return normalized; };
...
export function parseSolutionRow(value: unknown): SolutionRow | null {
  ...
  if (!isString(value.id) || !isString(value.author_id) || !isString(value.title) || !isString(value.summary) || !isString(value.description) || (!isString(value.category) || !value.category.trim() || value.category.trim().length > 80) || !isString(value.organization) || !isString(value.status) || !isString(value.maturity_level) || !isString(value.implementation_difficulty) || !isString(value.location) || !isString(value.country) || !isString(value.impact_metric) || !isString(value.created_at) || !isString(value.updated_at)) return null;
...
export function mapSolutionRowToDomain(row: SolutionRow): Solution {
  ...
  return { id: row.id, authorId: row.author_id, title: row.title, summary: row.summary, description: row.description, category: safeCategory(row.category), image: row.image_url || undefined, organization: row.organization, author: row.author_name || 'Autor não informado', createdAt: row.created_at, updatedAt: row.updated_at, status: safe(row.status, statuses, 'Proposta'), maturityLevel: safe(row.maturity_level, maturities, 'Ideia'), implementationDifficulty: safe(row.implementation_difficulty, difficulties, 'Baixa'), estimatedCost: row.estimated_cost || 'Não informado', implementationTime: row.implementation_time || 'Não informado', location: row.location, country: row.country, impactMetric: row.impact_metric, likes: row.likes, comments: row.comments, views: row.views, relatedProblemIds, tags: row.tags, evidenceLinks: row.evidence_links };
}
