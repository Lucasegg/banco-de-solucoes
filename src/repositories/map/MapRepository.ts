...
const unavailable = (operation: string, error: unknown): Result<never> => {
  console.error(`[MapRepository] ${operation}`, error);
  return { ok: false, message: 'Mapa temporariamente indisponível.' };
};
...
export class MapRepository {
  ...
  async getProblemsInBounds(bounds: MapBounds,filters: MapFilters={}): Promise<Result<MapProblem[]>> {
    ...
    if(error)return unavailable('get_problems_in_bounds', error);
  ...
  async getRegionSummary(): Promise<Result<ProblemRegionSummary[]>> { if(!this.client)return {ok:true,data:[]}; const {data,error}=await this.client.rpc('get_problem_region_summary'); if(error)return unavailable('get_problem_region_summary', error); return {ok:true,data:(data||[]).map((r:Record<string,unknown>)=>({state:String(r.state),city:String(r.city),totalProblems:Number(r.total_problems),inProgress:Number(r.in_progress),resolved:Number(r.resolved),lastUpdated:String(r.last_updated)}))}; }
  async getProblemLocation(id:string):Promise<Result<MapProblem|null>> { if(!this.client)return {ok:true,data:localMapProblems().find(p=>p.id===id)||null}; const {data,error}=await this.client.rpc('get_public_problem_location',{p_problem_id:id});if(error)return unavailable('get_public_problem_location', error);const row=data?.[0];if(!row)return {ok:true,data:null};return {ok:true,data:{id:row.id,title:row.title,category:row.category,status:row.status,city:row.city,state:row.state,location:{latitude:row.latitude,longitude:row.longitude,precision:row.geolocation_precision},updatedAt:row.updated_at,verified:Boolean(row.source_verified_at)} as MapProblem}; }
}
