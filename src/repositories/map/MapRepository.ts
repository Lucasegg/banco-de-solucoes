...
export class MapRepository {
  ...
  async getRegionSummary(): Promise<Result<ProblemRegionSummary[]>> { if(!this.client)return {ok:true,data:[]}; const {data,error}=await this.client.rpc('get_problem_region_summary'); if(error){logger.error('Falha ao carregar resumo regional',error,{operation:'get_problem_region_summary'});return {ok:false,message:'Não foi possível carregar o resumo das regiões. Tente novamente.'};} return {ok:true,data:(data||[]).map((r:Record<string,unknown>)=>({state:String(r.state),city:String(r.city),totalProblems:Number(r.total_problems),inProgress:Number(r.in_progress),resolved:Number(r.resolved),lastUpdated:String(r.last_updated)}))}; }
  async getProblemLocation(id:string):Promise<Result<MapProblem|null>> { if(!this.client)return {ok:true,data:localMapProblems().find(p=>p.id===id)||null}; const {data,error}=await this.client.rpc('get_public_problem_location',{p_problem_id:id});if(error){logger.error('Falha ao carregar localização pública',error,{operation:'get_public_problem_location'});return {ok:false,message:'Não foi possível carregar a localização deste problema. Tente novamente.'};}const row=data?.[0];if(!row)return {ok:true,data:null};return {ok:true,data:{id:row.id,title:row.title,category:row.category,status:row.status,city:row.city,state:row.state,location:{latitude:row.latitude,longitude:row.longitude,precision:row.geolocation_precision},updatedAt:row.updated_at,verified:Boolean(row.source_verified_at)} as MapProblem}; }
}
