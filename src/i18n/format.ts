...
export const formatCivilDateUtc = (value: string | number | Date | null | undefined, locale: SupportedLocale, fallback = '—') => { const date = validDate(value); return date ? new Intl.DateTimeFormat(locale, { timeZone: 'UTC' }).format(date) : fallback; };
...
export const formatNumber = (value: number | null | undefined, locale: SupportedLocale, options?: Intl.NumberFormatOptions, fallback = '—') => Number.isFinite(value) ? new Intl.NumberFormat(locale, options).format(value as number) : fallback;
