...
export function ChangeUserRoleDialog({ user, onClose, onConfirm }: Props) {
  ...
  useEffect(() => { closeButton.current?.focus(); const onKeyDown = (event: KeyboardEvent) => { if (event.key === 'Escape' && !busy) onClose(); }; window.addEventListener('keydown', onKeyDown); return () => window.removeEventListener('keydown', onKeyDown); }, [busy, onClose]);
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-4" role="presentation" onMouseDown={(event: { target: unknown; currentTarget: HTMLDivElement }) => { if (event.target === event.currentTarget && !busy) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="change-role-title" aria-describedby="change-role-description" className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-xl dark:bg-slate-900">
