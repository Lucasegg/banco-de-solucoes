...
export function InstitutionalFooter({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  return <footer aria-label={t('footer.label')} className="border-t border-slate-200 bg-slate-950 text-slate-200">
    <div className="mx-auto grid max-w-7xl gap-8 px-6 py-10 md:grid-cols-[1fr_auto] md:items-start">
      ...
      <nav aria-label={t('footer.label')}><ul className="grid gap-3 text-sm sm:grid-cols-2">
        {links.map(([page, label]) => <li key={page}><button className="rounded-sm text-left underline decoration-slate-500 underline-offset-4 hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white" onClick={() => onNavigate(page)}>{t(label)}</button></li>)}
      </ul></nav>
