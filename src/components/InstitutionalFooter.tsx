...
export function InstitutionalFooter() {
  ...
  return <footer aria-label={t('footer.label')} className="border-t border-slate-200 bg-slate-950 text-slate-200">
    <div className="mx-auto grid max-w-7xl gap-8 px-6 py-10 md:grid-cols-[1fr_auto] md:items-start">
      ...
      <p className="border-t border-slate-700 pt-6 text-xs text-slate-400 md:col-span-2">{t('footer.copyright', { year: new Date().getFullYear() })}{' '}<a href="https://www.linkedin.com/in/lucas-gomes-da-silva-1407/" target="_blank" rel="noreferrer noopener" className="underline decoration-slate-500 underline-offset-4 hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white">{t('footer.creator')}</a></p>
    </div>
  </footer>;
}
