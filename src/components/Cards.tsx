  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-line bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-soft">
      ...
      <div className="p-6">
        ...
        {problem.importedFromExternalSource && <div className="mt-4 rounded-2xl border border-teal-100 bg-teal-50 p-3 text-xs text-teal-900"><strong>{t('card.verifiedSource')}</strong><span className="mt-1 block">{problem.sourceName}</span>{problem.sourceUrl && <a href={problem.sourceUrl} target="_blank" rel="noopener noreferrer" onClick={(event: { stopPropagation: () => void }) => event.stopPropagation()} className="mt-2 inline-flex items-center gap-1 font-semibold underline">{t('card.originalSource')} <ExternalLink size={13} /></a>}</div>}
        <div className="mt-5 grid grid-cols-3 gap-2 text-xs text-slate-500">
          <Metric icon={<Heart size={15} />} value={formatNumber(problem.likes, locale)} label={t('metric.likes')} />
          <Metric icon={<MessageCircle size={15} />} value={formatNumber(problem.comments, locale)} label={t('metric.comments')} />
          <Metric icon={<Eye size={15} />} value={formatNumber(problem.views, locale)} label={t('metric.views')} />
        </div>
        <button className="mt-6 inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 py-2 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-slate-400" onClick={() => onOpen(problem.id)}>
          {t('card.details')} <ArrowRight size={16} className="transition group-hover:translate-x-1" />
        </button>
      </div>
    </article>
  );
  ...
  return (
    <article className="group overflow-hidden rounded-3xl border border-teal-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-soft">
      ...
      <div className="p-6">
        ...
        <div className="mt-5 grid grid-cols-3 gap-2 text-xs text-slate-500">
          <Metric icon={<Heart size={15} />} value={formatNumber(solution.likes, locale)} label={t('metric.likes')} />
          <Metric icon={<MessageCircle size={15} />} value={formatNumber(solution.comments, locale)} label={t('metric.comments')} />
          <Metric icon={<Eye size={15} />} value={formatNumber(solution.views, locale)} label={t('metric.views')} />
        </div>
        <button className="mt-6 inline-flex items-center gap-2 rounded-full bg-teal-700 px-4 py-2 text-sm font-semibold text-white focus:outline-none focus:ring-2 focus:ring-teal-400" onClick={() => onOpen(solution.id)}>{t('card.details')} <ArrowRight size={16} className="transition group-hover:translate-x-1" /></button>
      </div>
    </article>
  );
}
