-- Sprint 45: Realtime delivery, private preferences and user-controlled retention.
begin;
...
begin
  ...
  return query select p.contributions,p.comments,p.favorites,p.updated_at
    from public.notification_preferences p where p.user_id=v_user_id;
end $$;

create function public.update_my_notification_preferences(
  p_contributions boolean, p_comments boolean, p_favorites boolean
) returns boolean
language plpgsql security definer set search_path = pg_catalog, public as $$
declare v_user_id uuid := auth.uid();
begin
  if v_user_id is null then raise exception 'Authentication required' using errcode='42501'; end if;
  if p_contributions is null or p_comments is null or p_favorites is null then
    raise exception 'Invalid preferences' using errcode='22023';
  end if;
  insert into public.notification_preferences(user_id,contributions,comments,favorites)
    values(v_user_id,p_contributions,p_comments,p_favorites)
  on conflict(user_id) do update set contributions=excluded.contributions,
    comments=excluded.comments,favorites=excluded.favorites,updated_at=now();
  return true;
end $$;
...
begin
  if v_user_id is null then raise exception 'Authentication required' using errcode='42501'; end if;
  delete from public.notifications n where n.recipient_id=v_user_id
    and n.read_at is not null and n.created_at < now() - interval '30 days';
...
revoke all on function public.get_my_notification_preferences(),
  ...
  public.delete_my_old_read_notifications() from public,anon;
grant execute on function public.get_my_notification_preferences(),
  public.update_my_notification_preferences(boolean,boolean,boolean),
  public.delete_my_old_read_notifications() to authenticated;
