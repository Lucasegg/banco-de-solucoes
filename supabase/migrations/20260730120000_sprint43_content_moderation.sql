...
declare
 ...
 v_last_action text;
 v_last_previous_status text;
 v_last_resulting_status text;
 v_reason text:=btrim(coalesce(p_reason,''));
 v_note text:=nullif(btrim(p_moderator_note),'');
 v_action_id uuid;
begin
 ...
 v_archived_status:=case when p_target_type='problem' then 'Arquivado' else 'Arquivada' end;
 ...
 if p_target_type='problem' then select p.status into v_current_status from public.problems p where p.id=p_target_id for update;
 else select s.status into v_current_status from public.solutions s where s.id=p_target_id for update;end if;
 if not found then raise exception 'Content not found' using errcode='P0002';end if;
 if p_action='archive' then
   if v_current_status=v_archived_status then raise exception 'Content is already archived' using errcode='23514';end if;
   v_result_status:=v_archived_status;
 else
   if v_current_status<>v_archived_status then raise exception 'Content is not archived' using errcode='23514';end if;
   select a.action,a.previous_status,a.resulting_status
 ...
 if p_target_type='problem' then update public.problems p set status=v_result_status where p.id=p_target_id;else update public.solutions s set status=v_result_status where s.id=p_target_id;end if;
 insert into public.content_moderation_actions(target_type,target_id,report_id,moderator_id,action,reason,moderator_note,previous_status,resulting_status) values(p_target_type,p_target_id,p_report_id,v_actor,p_action,v_reason,v_note,v_current_status,v_result_status) returning content_moderation_actions.id into v_action_id;
 return query select a.id,a.target_type,a.target_id,a.action,a.previous_status,a.resulting_status,a.created_at from public.content_moderation_actions a where a.id=v_action_id;
end $$;

create function public.get_content_moderation_state(p_target_type text,p_target_id uuid)
returns table(target_type text,target_id uuid,current_status text)
...
begin
 ...
 if p_target_type not in ('problem','solution') then raise exception 'Invalid content target' using errcode='22023';end if;
 if p_target_type='problem' then select p.status into v_current_status from public.problems p where p.id=p_target_id;
 else select s.status into v_current_status from public.solutions s where s.id=p_target_id;end if;
 if not found then raise exception 'Content not found' using errcode='P0002';end if;
 return query select p_target_type,p_target_id,v_current_status;
end $$;
