...
-- Private mathematical contract. Public RPCs inline the same expression so callers
-- never need EXECUTE on either distance helper.
...
returns double precision language sql immutable strict parallel safe security invoker set search_path=pg_catalog
...
with origin as (select * from public.problems where problems.id=p_problem_id and status<>'Arquivado'), candidates as (
 select p.*,o.category origin_category,o.city origin_city,o.state origin_state,
  ...
  case when public.public_problem_coordinate(o.latitude,o.geolocation_precision) is not null and public.public_problem_coordinate(p.latitude,p.geolocation_precision) is not null then 6371.0088*2*asin(least(1.0,sqrt(power(sin(radians(public.public_problem_coordinate(p.latitude,p.geolocation_precision)-public.public_problem_coordinate(o.latitude,o.geolocation_precision))/2),2)+cos(radians(public.public_problem_coordinate(o.latitude,o.geolocation_precision)))*cos(radians(public.public_problem_coordinate(p.latitude,p.geolocation_precision)))*power(sin(radians(public.public_problem_coordinate(p.longitude,p.geolocation_precision)-public.public_problem_coordinate(o.longitude,o.geolocation_precision))/2),2)))) end distance
 from public.problems p cross join origin o where p.id<>o.id and p.status<>'Arquivado'
), scored as (select c.*,greatest(0,(case when category=origin_category then 35 else 0 end)+(least(shared_tags,5)*8)+(least(text_rank,1)*25)+(case when city=origin_city then 10 when state=origin_state then 5 else 0 end)+(case when distance<=10 then 10 when distance<=50 then 5 else 0 end)+least(5,ln(1::numeric+greatest(coalesce(likes,0),0)::numeric)) )::numeric(8,3) score,
...
with origin as (select * from public.problems where problems.id=p_problem_id and status<>'Arquivado'), candidates as (
 ...
 case when public.public_problem_coordinate(o.latitude,o.geolocation_precision) is not null and s.latitude is not null then 6371.0088*2*asin(least(1.0,sqrt(power(sin(radians(s.latitude-public.public_problem_coordinate(o.latitude,o.geolocation_precision))/2),2)+cos(radians(public.public_problem_coordinate(o.latitude,o.geolocation_precision)))*cos(radians(s.latitude))*power(sin(radians(s.longitude-public.public_problem_coordinate(o.longitude,o.geolocation_precision))/2),2)))) end distance
...
with origin as(select * from public.solutions where solutions.id=p_solution_id and status<>'Arquivada'), candidates as(select p.*,o.category origin_category,
 (select count(distinct t) from unnest(coalesce(p.tags,'{}')) t where t=any(coalesce(o.tags,'{}'))) shared_tags,
 ts_rank_cd(to_tsvector('portuguese',coalesce(p.title,'')||' '||coalesce(p.summary,'')||' '||coalesce(p.description,'')),public.safe_search_tsquery(coalesce(o.title,'')||' '||coalesce(o.summary,''))) text_rank,
 case when o.latitude is not null and public.public_problem_coordinate(p.latitude,p.geolocation_precision) is not null then 6371.0088*2*asin(least(1.0,sqrt(power(sin(radians(public.public_problem_coordinate(p.latitude,p.geolocation_precision)-o.latitude)/2),2)+cos(radians(o.latitude))*cos(radians(public.public_problem_coordinate(p.latitude,p.geolocation_precision)))*power(sin(radians(public.public_problem_coordinate(p.longitude,p.geolocation_precision)-o.longitude)/2),2)))) end distance
 from public.problems p cross join origin o where p.status<>'Arquivado' and not exists(select 1 from public.solution_problems sp where sp.solution_id=o.id and sp.problem_id=p.id)), scored as(select c.*,greatest(0,(case when category=origin_category then 35 else 0 end)+(least(shared_tags,5)*8)+(least(text_rank,1)*25)+(case when distance<=10 then 10 when distance<=50 then 5 else 0 end)+least(5,ln(1::numeric+greatest(coalesce(likes,0),0)::numeric)))::numeric(8,3) score,array_remove(array[case when category=origin_category then 'Mesma categoria' end,case when shared_tags>0 then shared_tags||case when shared_tags=1 then ' tag em comum' else ' tags em comum' end end,case when text_rank>0.05 then 'Descrição semelhante' end,case when distance<=50 then 'Próximo da localização da solução' end],null) reasons from candidates c), numbered as(select *,count(*) over() n from scored where score>0)
select id,title,coalesce(summary,left(description,180)),category,tags,city,state,updated_at,score,reasons,n from numbered order by score desc,updated_at desc,id asc limit least(greatest(coalesce(p_limit,6),1),24) offset greatest(coalesce(p_offset,0),0)
$$;
