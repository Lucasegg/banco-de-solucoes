...
Sem Supabase, o `MapRepository` lê a coleção `problems` já usada pelo armazenamento local, descarta itens sem latitude, longitude e precisão válidas e aplica os mesmos bounds, filtros e limite de 200, sem fabricar conteúdo. A cartografia e a biblioteca são carregadas de endpoints públicos e, portanto, o fundo do mapa depende de conectividade. Validações integradas das RPCs, grants e policies exigem um projeto Supabase com a migration aplicada.

### Hotfix 25.1 — instalação e cache da RPC

O mapa depende do Leaflet 1.9.4 e das migrations `20260717250000_public_problem_map.sql` e `20260717251000_hotfix_map_rpc_cache.sql`, que devem ser aplicadas em ordem com `supabase db push` (ou pelo fluxo de migrations do projeto). O hotfix verifica a assinatura completa de `get_problems_in_bounds` e envia `NOTIFY pgrst, 'reload schema'`, fazendo o PostgREST atualizar seu cache. Depois de qualquer alteração manual em uma RPC, um administrador do banco pode executar novamente `NOTIFY pgrst, 'reload schema';`. Se a função ainda estiver ausente ou o cache não puder ser atualizado, a interface mantém a cartografia vazia e apresenta uma mensagem pública sem detalhes técnicos; o erro original fica somente no console do navegador.
