...
do $$ declare marked_count integer;remaining_unread bigint;begin
 ...
 select public.mark_all_my_notifications_read() into marked_count;
 select public.get_my_unread_notification_count() into remaining_unread;
 if marked_count<>1 or remaining_unread<>0 then raise exception 'batch mark failed: marked %, remaining %',marked_count,remaining_unread;end if;
end $$;
reset role;
